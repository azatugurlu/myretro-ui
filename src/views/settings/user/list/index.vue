    ...
    <el-main>
      ...
      <el-card class="box-card">
        <el-table v-loading="listLoading" :data="list" element-loading-text="Loading" border fit highlight-current-row>
          ...
          <el-table-column align="center" :label="$t('userSettings.fullName')">
            <template slot-scope="scope">
    ...
    <el-dialog :title="$t('userSettings.updateDialogTitle')" :visible.sync="dialogVisible" width="50%">
      <el-table v-loading="dilaogLoading" :data="userRoles" element-loading-text="Loading" border fit highlight-current-row>
        ...
        <el-table-column align="center" :label="$t('userSettings.activity')">
          <template slot-scope="scope">
            <el-switch :disabled="scope.row.name == 'role_courier'" v-model="scope.row.active" @change="roleChanged(scope.row)" />
          </template>
        </el-table-column>
      </el-table>
...
<script>
import { getUsers, getRoles, createUserRole, deleteUserRole, updateUserInformationByUserId } from '@/api/userDetails'
...
export default {
  ...
  methods: {
    ...
    fetchData(page, perPage, sortField,filter) {
      ...
      getUsers(page, perPage, sortField,filter).then(response => {
    ...
    roleChanged(role) {
      ...
      if (role.active == true) {
        createUserRole(this.selectedUser.id, userRole).then(response => {
            this.listLoading = false
            Message({
              message: this.$t('userSettings.roleAssigned'),
              type: 'info',
              duration: 2 * 1000
            })
          },
        ...
        deleteUserRole(this.selectedUser.id, role.id).then(response => {
            this.listLoading = false
            Message({
              message: this.$t('userSettings.roleDropped'),
              type: 'info',
              duration: 2 * 1000
            })
          },
    ...
    statusChanged(user) {
      ...
      updateUserInformationByUserId(userDto, user.id).then(response => {
          this.listLoading = false
          Message({
    ...
    transactionList(user){
      this.$router.push({name: 'Transactions', params: { id: user.id }})
    }
  }
}
</script>
